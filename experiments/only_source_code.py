...
from model.only_source_code import SourceCodeProcessModel
...
def train(model, loader, criterion, optimizer, device):
    ...
    for batch_id, batch in enumerate(loader):
        ...
        out = model(vectors)  # 输出: [batch_size]
...
def run_all_vulnerabilities(vul_list, base_path="../dataset", epochs=50, batch_size=16, hidden_dim=768, vocab_size=165,
    ...
    for vul in vul_list:
        ...
        for run_id in range(num_runs):
            ...
            data_list = load_data(source_data_path, name_path, label_path)
            ...
            model = SourceCodeProcessModel(hidden_dim=hidden_dim, num_layers=num_layers,
                                  dropout=dropout,pool_type=pool_type).to(device)

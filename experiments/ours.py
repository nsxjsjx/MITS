...
from model.ours import MultiModeModel
...
# def compute_distill_loss(logits_student, logits_teacher, info_learn_source, info_learn_source_real, labels, criterion):
#     loss_std = criterion(logits_student, labels.float())
#     if logits_teacher is not None:
#         loss_tch = criterion(logits_teacher, labels.float())
#         loss_align = 1 - F.cosine_similarity(info_learn_source, info_learn_source_real, dim=1).mean()
#         loss = loss_std + loss_tch + loss_align
#     else:
#         loss = loss_std
#     return loss
...
def compute_distill_loss(
        logits_student,
        logits_teacher,
        info_learn_source_pred,
        info_learn_source_real,
        labels,
        criterion=nn.BCEWithLogitsLoss(),
        alpha=1,  # logits soft label KL loss 权重
        beta=1,  # 特征对齐 MSE 权重
        gamma=1,  # logits 直接对齐（MSE）权重 ← 新增
        temperature=2.0
):
    # 原始监督损失（学生 vs ground truth）
    loss_std = criterion(logits_student, labels.float())
    ...
    if logits_teacher is not None:
        # logits soft label 蒸馏（KL 散度）
        # prob_std = F.log_softmax(logits_student / temperature, dim=-1)
        # prob_tch = F.softmax(logits_teacher / temperature, dim=-1)
        # loss_kl = F.kl_div(prob_std, prob_tch, reduction='batchmean') * (temperature ** 2)
        loss_kl=criterion(logits_teacher, labels.float())

        # logits 直接对齐（MSE）
        loss_logits_mse = F.mse_loss(logits_student, logits_teacher)

        # 特征蒸馏（源码向量对齐）
        loss_feature = F.mse_loss(info_learn_source_pred, info_learn_source_real)

        # 综合
        loss += alpha * loss_kl + beta * loss_feature + gamma * loss_logits_mse
...
def train(model, loader, criterion, optimizer, device):
    ...
    for batch_id, batch in enumerate(loader):
        ...
        logits_student, logits_teacher, info_learn_source, info_learn_source_real = model(data_graph, data_byte,
                                                                                          data_source)

        loss = compute_distill_loss(logits_student, logits_teacher, info_learn_source, info_learn_source_real, labels,
                                    criterion)
        ...
        # 收集标签和预测
        preds = (torch.sigmoid(logits_teacher) > 0.5).int().cpu().tolist()
        # preds = (torch.sigmoid(logits_student) > 0.5).int().cpu().tolist()
...
def test(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            logits_student = model(data_graph, data_byte, None)
            loss = compute_distill_loss(logits_student, None, None, None, labels, criterion)
...
def run_all_vulnerabilities(config_model,
    ...
    for vul in vul_list:
        ...
        for run_id in range(num_runs):
            ...
            model = MultiModeModel(**config_model).to(device)
...
if __name__ == "__main__":
    ...
    with open(log_path, "a", encoding="utf-8") as f:
        f.write("==== Config ====\n")
        for key, value in {**config_run, **config_model, **config_not_use}.items():
            f.write(f"{key}: {value}\n")
        f.write("==== Results ====\n")

...
# ====== 数据加载函数 ======
from model.ablation_wo_seq import MultiModeModel
...
def run_all_vulnerabilities(config_model,
    ...
    for vul in vul_list:
        ...
        for run_id in range(num_runs):
            ...
            for epoch in range(1, epochs + 1):
                ...
                if test_f1 > best_f1:
                    best_f1 = test_f1
                    # todo
                    torch.save(model_std.state_dict(), f"../model_save/{model_save_dir}/without_seq/{vul}_{time_str}.pth")
                    counter = 0
                else:
                    counter += 1
                    if counter >= patience:
            ...
            plot_training_curves(
                ...
                vul=vul,
                # todo
                save_path=f'../plots/{model_save_dir}/without_seq/{vul}_{time_str}.png'
            )

            print(f"Start testing for {vul} Run {run_id + 1}.\n")
            # todo
            model_std.load_state_dict(torch.load(f"../model_save/{model_save_dir}/without_seq/{vul}_{time_str}.pth"))
            test_loss, test_acc, test_prec, test_rec, test_f1 = test(model_std, test_loader, criterion, device,model_tch)
            metrics_list.append({
                'loss': test_loss,
                'acc': test_acc,
        ...
        # todo 写入文件
        with open(f"../model_save/{model_save_dir}/without_seq/result.txt", "a", encoding="utf-8") as f:
            f.write(result_str)
...
if __name__ == "__main__":
    ...
    # 写日志文件（追加模式）todo
    log_path = f"../model_save/{config_run['model_save_dir']}/without_seq/result.txt"

...
def train(model_std, loader, criterion, optimizer, device, model_tch):
    ...
    for batch_id, batch in enumerate(loader):
        ...
        # teacher 使用 data 的拷贝（避免被 student 修改影响）
...
def run_all_vulnerabilities(config_model,
    ...
    for vul in vul_list:
        ...
        for run_id in range(num_runs):
            ...
            model_tch.load_state_dict(torch.load(f"../model_save/{model_save_dir}_tch_test/{vul}_best_model.pth"))
            ...
            for epoch in range(1, epochs + 1):
                ...
                if test_f1 > best_f1:
                    best_f1 = test_f1
                    torch.save(model_std.state_dict(), f"../model_save/{model_save_dir}/{vul}_best_model.pth")
                    counter = 0
                else:
                    counter += 1
                    if counter >= patience:
            ...
            model_std.load_state_dict(torch.load(f"../model_save/{model_save_dir}/{vul}_best_model.pth"))
            test_loss, test_acc, test_prec, test_rec, test_f1 = test(model_std, test_loader, criterion, device,model_tch)
            metrics_list.append({
                'loss': test_loss,
                'acc': test_acc,

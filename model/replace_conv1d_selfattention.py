...
from model.replace import replace_conv1d
...
class MultiModeModel(nn.Module):
    def __init__(self, vocab_size=165,
        ...
        #     nn.Dropout(0.2)
        # )
        # self.process_token_node = replace_conv1d.MeanPooling()
        self.process_token_node = replace_conv1d.SelfAttention1D(hidden_dim_graph)
        # self.process_token_node = replace_conv1d.LinearProject(hidden_dim_graph)
        # 图结点的特征提取
    ...
    def forward(self, data_graph, data_seq, data_source=None):
        ...
        # 融合特征（学生模型）
        fusion_std = torch.stack([info_learn_graph, info_learn_seq, info_learn_byte_to_source], dim=1)
        # max池化

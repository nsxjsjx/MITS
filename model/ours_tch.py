...
class MultiModeModelTCH(nn.Module):
    def __init__(self, vocab_size=165,
        ...
        # 初始化可学习参数（注意力分数向量），大小为 模态数量
        self.attn_weights = nn.Parameter(torch.randn(3))
        ...
        # 2. 拼接的分类器
        self.classifier = nn.Linear(input_dim_classifier*3, 1)
    ...
    def forward(self, data_graph, data_seq, data_source=None):
        # 图
        embed_graph = self.embedding_graph(data_graph.x.long()).permute(0, 2, 1)
        ...
        info_learn_source_real = self.net_source(data_source)

        features_tch = torch.stack([info_learn_graph, info_learn_seq, info_learn_source_real], dim=1)
        # 1. 最大池化
        # features_tch_pooled, _ = torch.max(features_tch, dim=1)
        # 2. 平均池化
        # features_tch_pooled = torch.mean(features_tch, dim=1)
        # 3. 注意力池化
        # # 对权重进行 softmax，使其归一化为概率
        # attn = F.softmax(self.attn_weights, dim=0)  # (num_modalities,)
        # # 加权求和池化
        # features_tch_pooled = torch.sum(features_tch * attn.view(1, -1, 1), dim=1)  # (batch_size, feature_dim)
        # 4. 拼接
        features_tch_pooled = torch.cat([info_learn_graph, info_learn_seq, info_learn_source_real], dim=1)

        logits_tch = self.classifier(features_tch_pooled)

        return logits_tch.squeeze(1)
